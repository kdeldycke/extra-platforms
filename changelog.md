...
- Fix interpreter-hosted shell detection (`_interpreter_shell`) on Windows: `PurePosixPath` was used to extract the launcher basename, so Windows paths with backslash separators (like `C:\Users\...\xonsh`) were never recognized. Replaced with `Path`, which uses the native separator on each platform.
- Fix `_running_shell_path` returning `None` on Windows for paths produced by the POSIX process-tree walk (which uses `/`-prefixed paths): `os.path.isabs` returns `False` for such paths on Windows. The absoluteness check now also accepts paths starting with `/`.
