...
- Recognize shells running under a user-mode CPU emulator. Under foreign-architecture emulation (like `docker run --platform linux/arm64` on an x86 host), the process tree shows `qemu-aarch64 /bin/bash …` rather than `bash`; the walk now strips a leading `qemu-<arch>` (or `rosetta`) wrapper and reads the emulated command, composing with the interpreter-hosted detection (an emulated `python` running Xonsh still resolves to Xonsh). Detection still degrades to the `SHELL`/version-variable signals when the tree is unavailable.
