...
- **CI-only pytest flags belong in workflow steps, not `[tool.pytest].addopts`.** Flags like `--cov-report=xml` produce artifacts only needed in CI. Placing them in `addopts` pollutes local test runs. Keep `addopts` for flags that apply everywhere (`--cov`, `--cov-report=term`, `--dist`, `--durations`, `--numprocesses`). Pass CI-specific flags in the workflow `run:` step.
- **Coverage configuration belongs in `[tool.coverage]`.** Use the `[tool.coverage]` section in `pyproject.toml` for `run.branch`, `run.source`, and `report.precision` instead of flags in `addopts`. Of the coverage flags, `addopts` carries only `--cov` (to activate the plugin) and `--cov-report=term` (for local feedback).
