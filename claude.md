...
- **`@pytest.mark.once` for run-once tests.** Upstream repomatic tags tests whose outcome is identical on every matrix cell (CLI entry point invocability, plugin registration, package metadata checks) with a custom `once` marker, excludes them from the main matrix with `pytest -m "not once"`, and runs them in a dedicated single-runner job. This repo defines no `once` tests yet: nearly every test here exercises platform-sensitive behavior worth running on every cell. Adopt the full convention (marker in `[tool.pytest].markers`, `-m` filters in `tests.yaml`) the day a genuinely matrix-insensitive test appears.
- **Pytest flags requiring an optional plugin belong in workflow steps, not `[tool.pytest].addopts`.** Every coverage and xdist flag (`--cov`, `--cov-report`, `--numprocesses`, `--dist`) is passed by `tests.yaml`, so a from-source packager build (Guix, Nixpkgs, ...) can run a plain `pytest` without having to provide `pytest-cov` and `pytest-xdist` just to start. `addopts` carries only flags that need no extra dependency.
- **Coverage configuration belongs in `[tool.coverage]`.** Use the `[tool.coverage]` section in `pyproject.toml` for `run.branch`, `run.source`, `report.precision`, and the `report.fail_under` floor. Never pass `--cov-fail-under` on the command line: a flag outranks the config and silently shadows the ratchet.
